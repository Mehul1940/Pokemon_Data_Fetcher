const URL = "https://pokeapi.co/api/v2/";

class Pokemon {
  constructor(name) {
    this.name = name;
    this.data = null;
    this.speciesData = null;
    this.evolutionChainData = null;
    this.growthRateData = null;
    this.natureData = null;
    this.statsData = null;
    this.typeData = null;
    this.weaknesses = [];
    this.moves = [];
    this.typeAdvantages = [];
    this.typeDisadvantages = [];
  }

  // Fetch basic Pokémon data
  async fetchData() {
    try {
      const response = await fetch(`${URL}pokemon/${this.name}`);
      if (!response.ok) {
        throw new Error(`Pokémon not found: ${this.name}`);
      }
      this.data = await response.json();
    } catch (error) {
      console.error("Error fetching Pokémon data:", error);
      throw error;
    }
  }

  // Fetch species data (includes evolution chain URL)
  async fetchSpeciesData() {
    try {
      if (!this.data) {
        await this.fetchData();
      }
      const response = await fetch(this.data.species.url);
      if (!response.ok) {
        throw new Error(`Species data not found for: ${this.name}`);
      }
      this.speciesData = await response.json();
    } catch (error) {
      console.error("Error fetching species data:", error);
      throw error;
    }
  }

  // Fetch evolution chain data
  async fetchEvolutionChain() {
    try {
      if (!this.speciesData) {
        await this.fetchSpeciesData();
      }
      const response = await fetch(this.speciesData.evolution_chain.url);
      if (!response.ok) {
        throw new Error(`Evolution chain not found for: ${this.name}`);
      }
      this.evolutionChainData = await response.json();
    } catch (error) {
      console.error("Error fetching evolution chain:", error);
      throw error;
    }
  }

  // Fetch growth rate data
  async fetchGrowthRateData() {
    try {
      if (!this.speciesData) {
        await this.fetchSpeciesData();
      }
      const response = await fetch(this.speciesData.growth_rate.url);
      if (!response.ok) {
        throw new Error(`Growth rate data not found for: ${this.name}`);
      }
      this.growthRateData = await response.json();
    } catch (error) {
      console.error("Error fetching growth rate data:", error);
      throw error;
    }
  }

  // Fetch nature data
  async fetchNatureData() {
    try {
      const response = await fetch(`${URL}nature/`);
      if (!response.ok) {
        throw new Error(`Nature data not found`);
      }
      this.natureData = await response.json();
    } catch (error) {
      console.error("Error fetching nature data:", error);
      throw error;
    }
  }

  // Fetch stats data
  async fetchStatsData() {
    try {
      if (!this.data) {
        await this.fetchData();
      }
      this.statsData = this.data.stats;
    } catch (error) {
      console.error("Error fetching stats data:", error);
      throw error;
    }
  }

  // Fetch type data
  async fetchTypeData() {
    try {
      if (!this.data) {
        await this.fetchData();
      }
      this.typeData = this.data.types;
    } catch (error) {
      console.error("Error fetching type data:", error);
      throw error;
    }
  }

  // Fetch weaknesses
  async fetchWeaknesses() {
    try {
      if (!this.typeData) {
        await this.fetchTypeData();
      }
      
      // First get all types this pokemon has
      const types = this.typeData.map(type => type.type.name);
      
      // Get damage relations for each type
      const typeRelations = await Promise.all(
        types.map(async type => {
          const response = await fetch(`${URL}type/${type}`);
          if (!response.ok) throw new Error(`Type data not found for: ${type}`);
          return await response.json();
        })
      );
      
      // Calculate weaknesses considering all types
      const weaknesses = new Map();
      
      // For each type the pokemon has
      for (const typeRelation of typeRelations) {
        const relations = typeRelation.damage_relations;
        
        // Double damage from (weaknesses)
        for (const type of relations.double_damage_from) {
          weaknesses.set(type.name, (weaknesses.get(type.name) || 0) + 2);
        }
        
        // Half damage from (resistances)
        for (const type of relations.half_damage_from) {
          weaknesses.set(type.name, (weaknesses.get(type.name) || 0) * 0.5);
        }
        
        // No damage from (immunities)
        for (const type of relations.no_damage_from) {
          weaknesses.set(type.name, 0);
        }
      }
      
      // Filter out resistances and immunities, only keep weaknesses (multiplier > 1)
      this.weaknesses = Array.from(weaknesses.entries())
        .filter(([_, multiplier]) => multiplier > 1)
        .map(([type, multiplier]) => {
          // For 4x weaknesses, indicate with "4x"
          return multiplier === 4 ? `${type} (4x)` : type;
        });
    } catch (error) {
      console.error("Error fetching weaknesses:", error);
      throw error;
    }
  }

  // Fetch moves with levels
  async fetchMoves() {
    try {
      if (!this.data) {
        await this.fetchData();
      }
      this.moves = this.data.moves.map((move) => ({
        name: move.move.name,
        level: move.version_group_details[0]?.level_learned_at || 0,
      }));
    } catch (error) {
      console.error("Error fetching moves:", error);
      throw error;
    }
  }

  // Fetch type advantages
  async fetchTypeAdvantages() {
    try {
      if (!this.typeData) {
        await this.fetchTypeData();
      }
      
      const types = this.typeData.map(type => type.type.name);
      const typeRelations = await Promise.all(
        types.map(async type => {
          const response = await fetch(`${URL}type/${type}`);
          if (!response.ok) throw new Error(`Type data not found for: ${type}`);
          return await response.json();
        })
      );
      
      const advantages = new Map();
      
      for (const typeRelation of typeRelations) {
        const relations = typeRelation.damage_relations;
        
        // Double damage to (advantages)
        for (const type of relations.double_damage_to) {
          advantages.set(type.name, (advantages.get(type.name) || 0 + 2);
        }
        
        // Half damage to (not very effective)
        for (const type of relations.half_damage_to) {
          advantages.set(type.name, (advantages.get(type.name) || 0 * 0.5);
        }
        
        // No damage to (no effect)
        for (const type of relations.no_damage_to) {
          advantages.set(type.name, 0);
        }
      }
      
      this.typeAdvantages = Array.from(advantages.entries())
        .filter(([_, multiplier]) => multiplier > 1)
        .map(([type, multiplier]) => {
          return multiplier === 4 ? `${type} (4x)` : type;
        });
    } catch (error) {
      console.error("Error fetching type advantages:", error);
      throw error;
    }
  }

  // Fetch type disadvantages (resistances)
  async fetchTypeDisadvantages() {
    try {
      if (!this.typeData) {
        await this.fetchTypeData();
      }
      
      const types = this.typeData.map(type => type.type.name);
      const typeRelations = await Promise.all(
        types.map(async type => {
          const response = await fetch(`${URL}type/${type}`);
          if (!response.ok) throw new Error(`Type data not found for: ${type}`);
          return await response.json();
        })
      );
      
      const resistances = new Map();
      
      for (const typeRelation of typeRelations) {
        const relations = typeRelation.damage_relations;
        
        // Half damage from (resistances)
        for (const type of relations.half_damage_from) {
          resistances.set(type.name, (resistances.get(type.name) || 1) * 0.5);
        }
        
        // No damage from (immunities)
        for (const type of relations.no_damage_from) {
          resistances.set(type.name, 0);
        }
        
        // Double damage from (weaknesses - we exclude these)
        for (const type of relations.double_damage_from) {
          if (!resistances.has(type.name)) {
            resistances.set(type.name, 1); // Neutral if not already set
          }
        }
      }
      
      this.typeDisadvantages = Array.from(resistances.entries())
        .filter(([_, multiplier]) => multiplier < 1)
        .map(([type, multiplier]) => {
          if (multiplier === 0) {
            return `${type} (Immune)`;
          } else if (multiplier === 0.25) {
            return `${type} (1/4x)`;
          } else {
            return `${type} (1/2x)`;
          }
        });
    } catch (error) {
      console.error("Error fetching type disadvantages:", error);
      throw error;
    }
  }

  // Get Pokémon name
  getName() {
    return this.data ? this.data.name : "Data not loaded";
  }

  // Get Pokémon abilities
  getAbilities() {
    return this.data
      ? this.data.abilities.map((ability) => ability.ability.name)
      : [];
  }

  // Get Pokémon evolution chain with levels
  async getEvolutionChain() {
    if (!this.evolutionChainData) {
      await this.fetchEvolutionChain();
    }
    return this._parseEvolutionChain(this.evolutionChainData.chain);
  }

  // Helper method to recursively parse evolution chain
  _parseEvolutionChain(chain, evolutions = []) {
    const evolutionEntry = {
      name: chain.species.name,
      details: []
    };

    // Add current pokemon to evolutions array
    evolutions.push(evolutionEntry);

    // Process each possible evolution path
    for (const evolution of chain.evolves_to) {
      const details = evolution.evolution_details[0];
      
      const triggerNames = {
        "level-up": "Level Up",
        trade: "Trade",
        "use-item": "Use Item",
        shed: "Party Space",
        spin: "Spin",
        other: "Special"
      };

      const trigger = triggerNames[details.trigger.name] || details.trigger.name;
      const conditions = [];

      if (details.min_level) conditions.push(`Level ${details.min_level}`);
      if (details.min_happiness) conditions.push(`Friendship ${details.min_happiness}+`);
      if (details.item) conditions.push(`${this._formatName(details.item.name)}`);
      if (details.held_item) conditions.push(`Hold ${this._formatName(details.held_item.name)}`);
      if (details.time_of_day) conditions.push(`At ${details.time_of_day}`);
      if (details.known_move_type) conditions.push(`Know ${this._formatName(details.known_move_type.name)} move`);
      if (details.known_move) conditions.push(`Know ${this._formatName(details.known_move.name)}`);
      if (details.location) conditions.push(`Near ${this._formatName(details.location.name)}`);
      if (details.needs_overworld_rain) conditions.push(`Raining`);
      if (details.gender) conditions.push(`Gender: ${details.gender === 1 ? "Female" : "Male"}`);
      if (details.relative_physical_stats !== null) {
        const stats = ["Attack < Defense", "Attack > Defense", "Attack = Defense"];
        conditions.push(stats[details.relative_physical_stats + 1]);
      }
      if (details.party_species) conditions.push(`With ${this._formatName(details.party_species.name)} in party`);
      if (details.party_type) conditions.push(`With ${this._formatName(details.party_type.name)} type in party`);
      if (details.turn_upside_down) conditions.push(`Turn device upside down`);

      evolutionEntry.details.push({
        to: evolution.species.name,
        trigger: trigger,
        conditions: conditions
      });

      // Recursively process next evolution
      this._parseEvolutionChain(evolution, evolutions);
    }

    return evolutions;
  }

  // Get Pokémon gender rate
  async getGenderRate() {
    if (!this.speciesData) {
      await this.fetchSpeciesData();
    }
    const genderRate = this.speciesData.gender_rate;
    if (genderRate === -1) {
      return "Genderless";
    } else if (genderRate === 0) {
      return "Always male";
    } else if (genderRate === 8) {
      return "Always female";
    } else {
      const femalePercentage = (genderRate / 8) * 100;
      const malePercentage = 100 - femalePercentage;
      return `${malePercentage}% male, ${femalePercentage}% female`;
    }
  }

  // Get Pokémon growth rate
  async getGrowthRate() {
    if (!this.growthRateData) {
      await this.fetchGrowthRateData();
    }
    return this.growthRateData ? this.growthRateData.name : "Unknown";
  }

  // Get Pokémon natures
  async getNatures() {
    if (!this.natureData) {
      await this.fetchNatureData();
    }
    return this.natureData
      ? this.natureData.results.map((nature) => nature.name)
      : [];
  }

  // Get Pokémon stats
  async getStats() {
    if (!this.statsData) {
      await this.fetchStatsData();
    }
    return this.statsData
      ? this.statsData.map((stat) => ({
          name: stat.stat.name,
          base_stat: stat.base_stat,
        }))
      : [];
  }

  // Get Pokémon types
  async getTypes() {
    if (!this.typeData) {
      await this.fetchTypeData();
    }
    return this.typeData ? this.typeData.map((type) => type.type.name) : [];
  }

  // Get Pokémon image URL
  getImageUrl() {
    return this.data ? this.data.sprites.front_default : "";
  }

  // Format hyphenated names
  _formatName(str) {
    return str
      .split("-")
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
      .join(" ");
  }
}

// UI Logic
document.getElementById("fetchData").addEventListener("click", async () => {
  const pokemonName = document
    .getElementById("pokemonName")
    .value.toLowerCase();
  if (!pokemonName) {
    alert("Please enter a Pokémon name.");
    return;
  }

  const pokemon = new Pokemon(pokemonName);

  try {
    // Show loading spinner
    document.querySelector(".loading").classList.remove("hidden");
    document.querySelector(".loading").classList.remove("d-none");

    // Fetch all data
    await Promise.all([
      pokemon.fetchData(),
      pokemon.fetchSpeciesData(),
      pokemon.fetchEvolutionChain(),
      pokemon.fetchGrowthRateData(),
      pokemon.fetchNatureData(),
      pokemon.fetchStatsData(),
      pokemon.fetchTypeData(),
      pokemon.fetchWeaknesses(),
      pokemon.fetchMoves(),
      pokemon.fetchTypeAdvantages(),
      pokemon.fetchTypeDisadvantages(),
    ]);

    // Display Pokémon name
    document.getElementById("pokemonNameDisplay").textContent =
      pokemon.getName();

    // Display Pokémon image
    const pokemonImage = document.getElementById("pokemonImage");
    pokemonImage.src = pokemon.getImageUrl();
    pokemonImage.style.display = "block";

    // Display abilities
    const abilitiesList = document.getElementById("abilitiesList");
    abilitiesList.innerHTML = pokemon
      .getAbilities()
      .map((ability) => `<li class="list-group-item data-item">${ability}</li>`)
      .join("");

    // Display evolution chain with levels
    const evolutionChain = document.getElementById("evolutionChain");
    const evolutions = await pokemon.getEvolutionChain();
    
    // Format the evolution chain for display
    let evolutionHTML = '';
    for (let i = 0; i < evolutions.length; i++) {
      const evolution = evolutions[i];
      
      if (i === 0) {
        // First in chain
        evolutionHTML += `<li class="list-group-item data-item">${evolution.name}</li>`;
      } else {
        // Subsequent evolutions with details
        const prevEvolution = evolutions[i-1];
        const details = prevEvolution.details.find(d => d.to === evolution.name);
        
        if (details) {
          evolutionHTML += `<li class="list-group-item data-item">→ ${evolution.name} (${details.trigger}${
            details.conditions.length > 0 ? `: ${details.conditions.join(', ')}` : ''
          })</li>`;
        }
      }
    }
    evolutionChain.innerHTML = evolutionHTML;

    // Display gender rate
    const genderRate = document.getElementById("genderRate");
    genderRate.textContent = await pokemon.getGenderRate();

    // Display growth rate
    const growthRate = document.getElementById("growthRate");
    growthRate.textContent = await pokemon.getGrowthRate();

    // Display natures
    const naturesList = document.getElementById("naturesList");
    const natures = await pokemon.getNatures();
    naturesList.innerHTML = natures
      .map((nature) => `<li class="list-group-item data-item">${nature}</li>`)
      .join("");

    // Display stats
    const statsList = document.getElementById("statsList");
    const stats = await pokemon.getStats();
    statsList.innerHTML = stats
      .map(
        (stat) =>
          `<li class="list-group-item data-item">${stat.name}: ${stat.base_stat}</li>`
      )
      .join("");

    // Display types
    const typesList = document.getElementById("typesList");
    const types = await pokemon.getTypes();
    typesList.innerHTML = types
      .map((type) => `<li class="list-group-item data-item">${type}</li>`)
      .join("");

    // Display weaknesses
    const weaknessesList = document.getElementById("weaknessesList");
    weaknessesList.innerHTML = pokemon.weaknesses.length > 0 
      ? pokemon.weaknesses
          .map((weakness) => `<li class="list-group-item data-item">${weakness}</li>`)
          .join("")
      : `<li class="list-group-item data-item">None</li>`;

    // Display moves with levels
    const movesList = document.getElementById("movesList");
    movesList.innerHTML = pokemon.moves
      .slice(0, 20) // Limit to 20 moves for display
      .map(
        (move) =>
          `<li class="list-group-item data-item">${move.name} (Level ${move.level})</li>`
      )
      .join("");

    // Display type advantages
    const typeAdvantagesList = document.getElementById("typeAdvantagesList");
    typeAdvantagesList.innerHTML = pokemon.typeAdvantages.length > 0
      ? pokemon.typeAdvantages
          .map((advantage) => `<li class="list-group-item data-item">${advantage}</li>`)
          .join("")
      : `<li class="list-group-item data-item">None</li>`;

    // Display type disadvantages
    const typeDisadvantagesList = document.getElementById("typeDisadvantagesList");
    typeDisadvantagesList.innerHTML = pokemon.typeDisadvantages.length > 0
      ? pokemon.typeDisadvantages
          .map((disadvantage) => `<li class="list-group-item data-item">${disadvantage}</li>`)
          .join("")
      : `<li class="list-group-item data-item">None</li>`;

    // Show the data section
    document.getElementById("pokemonData").classList.remove("hidden");
    document.getElementById("pokemonData").classList.remove("d-none");

    // Hide loading spinner
    document.querySelector(".loading").classList.add("hidden");
    document.querySelector(".loading").classList.add("d-none");
  } catch (error) {
    console.error("Error fetching Pokémon data:", error);
    alert(
      "Failed to fetch Pokémon data. Please check the name and try again."
    );

    // Hide loading spinner
    document.querySelector(".loading").classList.add("hidden");
    document.querySelector(".loading").classList.add("d-none");
  }
});